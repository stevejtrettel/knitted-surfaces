 ...
 * screenshot, and the full Geometry / Look / Render panel. Starts on a bundled
 * sample mesh; use "Load OBJ…" to swap in your own (connected, 2-colourable
 * quad mesh — see README constraints).
 */
...
function rebuild(): void {
  ...
  if (!geometry) return;
  const analysis = analyze(geometry, controls.pattern);
  const result = generateStrands(analysis, controls.pattern, controls.options);
  group = showTubes
...
geo.button('Load OBJ…', async () => {
  ...
  if (parsed) { geometry = { cellType: 'quad', mesh: parsed }; rebuild(); }
});

const look = panel.tab('Look');
const controls = buildPatternControls(look, 'quad', { value: 'loop', onChange: rebuild });
...
fetch(`${import.meta.env.BASE_URL}meshes/torus.obj`)
  .then((r) => r.text())
  .then((text) => { geometry = { cellType: 'quad', mesh: parseOBJ(text) }; rebuild(); })
